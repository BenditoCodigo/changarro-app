# Changarro - Documentación

Bienvenido a la documentación de Changarro, una aplicación de gestión de ventas
diarias para negocios locales.

Aquí encontrarás todo lo necesario para entender el proyecto, usar la herramienta
y contribuir a su desarrollo.

---

## Contenido

| #   | Documento                                                   | Descripción                                       |
| --- | ----------------------------------------------------------- | ------------------------------------------------- |
| 01  | [Visión](./01-vision.md)                                    | Hacia dónde va este proyecto                      |
| 02  | [Misión](./02-mision.md)                                    | Para qué existe y a quién sirve                   |
| 03  | [Política de Privacidad](./03-politica-de-privacidad.md)    | Compromiso de protección de datos y privacidad    |

### Subcarpetas

| Sección                                             | Descripción                                        |
| --------------------------------------------------- | -------------------------------------------------- |
| [Manuales de usuario](./manuales-usuario/README.md) | Guías para usar Changarro sin conocimiento técnico |
| [Manuales técnicos](./manuales-tecnicos/README.md)  | Guías para desarrolladores y colaboradores         |

---

## ¿Por dónde empezar?

- Si quieres entender **qué es** Changarro → [Misión](./02-mision.md)
- Si quieres **usar** la aplicación → [Manuales de usuario](./manuales-usuario/README.md)
- Si quieres **contribuir** al código → [Manuales técnicos](./manuales-tecnicos/README.md)
